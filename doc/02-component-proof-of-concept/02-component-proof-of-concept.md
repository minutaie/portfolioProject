...
After looking at all 3 of my component choices, I decided to implement Playlist. I chose to implement Playlist because it is practical and clearly structured. The kernel methods can remain pretty minimal, such as adding, removing, and checking the length. More complex behaviors such as shuffling and moving a song to the front can be layered in the secondary interface. This separation fits cleanly within the OSU software sequence discipline and allows for a meaningful range of method types. The design is simple enough to implement realistically, yet can demonstrate the overall feasibility and client usefulness of the component.
